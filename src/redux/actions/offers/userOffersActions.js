...
const setUserOffers = payload => ({ type: 'SET_USER_OFFERS', payload })
const resetUserOffers = () => ({ type: 'CLEAR_OFFERS' })

// filtering and ordering
export const fetchUserOffers = user_id => dispatch => {
    dispatch(resetUserOffers())

    API_Handler(false, {method: 'get', url: `offers/?ordering=creation_date&user_id=${user_id}`})
    .then(res => {
...
export const deleteOffer = (offer_id, user_id) => dispatch => {
    API_Handler(true, {method: 'delete', url: `offers/${offer_id}/`})
    .then(() => {
