...
const Chat = (props) => {
  ...
  useEffect(() => {
    waitForSocketConnection(() => {
      WebSocketInstance.fetchMessages(props.userId, props.chatId);
    });
  ...
  const waitForSocketConnection = (callback) => {
    setTimeout(() => {
      if (WebSocketInstance.state() === 1) callback();
  ...
  const renderMessages = (messages) => {
    return messages.map((message) => (
      <div
        className={
          message.author === props.userId ? 'message__author' : 'message__other'
        }
      >
        <Message
          color={message.author === props.userId ? 'blue' : null}
          style={
            message.author === props.userId
              ? { width: '45%', textAlign: 'right' }
              : { width: '45%' }
          }
        >
          {message.author !== props.userId ? (
            <Message.Header>{props.participantName}:</Message.Header>
          ) : (
            <Message.Header>ja:</Message.Header>
          )}
          <Message.Content style={{ marginTop: '5px', marginBottom: '5px' }}>
            {message.content}
          </Message.Content>
  ...
  return (
    <div>
      ...
      <div className="text_input">
        <TextArea
          value={message}
          style={{ width: '45%', margin: '0 0 10px auto', padding: '5px' }}
        ...
        <Button
          ...
          color="orange"
          style={{ width: '45%', marginRight: '0', marginLeft: 'auto' }}
        >
          Wyślij
        </Button>
      </div>
